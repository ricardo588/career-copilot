...
- Private, read-only JSON/CSV vacancy-export shortlist generation with selected-source, freshness and canonical-URL validation; it never queries a portal or changes a tracker
...
- [Portal Catalog Evidence 0.10.1 release brief](docs/ROADMAP-0.10.1.md)
- [Integrated Safe Acquisition Scope 0.11.0 roadmap](docs/ROADMAP-0.11.0.md)
- [Versioned portal coverage catalog](skills/career-copilot/references/portal-catalog.md)
- [Explicit acquisition and action contract](skills/career-copilot/references/acquisition-and-actions.md)
...
Version 0.11.0 adds explicit, read-only public acquisition for candidate-confirmed Greenhouse and Lever feeds, provider-specific normalized freshness, local catalog-freshness auditing, and draft-only external-action plans. It validates private output paths before any GET, rejects redirects, and accepts only documented provider-hosted job-page URLs; it never searches, authenticates, follows application URLs, submits applications, writes a tracker or contacts people. It retains v0.10.1 dated portal-catalog evidence. It retains transactional Gmail triage and the optional, local Obsidian Kanban projection from v0.9.0. It extends v0.9.1 strategic onboarding with transparent, user-editable job-portal coverage suggestions derived from declared role family, seniority, industry, eligible geography, work mode and employment type. The local catalog is a coverage heuristic, not a current market-performance ranking; its dated evidence and maintenance boundary are published in the [versioned portal coverage catalog](skills/career-copilot/references/portal-catalog.md). It also introduces private read-only JSON/CSV vacancy-export shortlisting: it accepts only an explicitly supplied local file, normalizes documented CSV headers and source labels locally, validates candidate-selected sources, posting freshness and canonical URLs, removes canonical duplicates, and preserves candidate context for later evaluation without declaring a match. It does not query portals, retrieve live vacancies, write the tracker, submit applications, contact people, or perform any external action. Target and portal choices and shortlist reports remain private. Any Obsidian mutation still requires `confirm_each_external`, a reviewed current-plan hash, private workspace auditing, current preflight, atomic write, and verified readback. Google adapters require a separately installed and authenticated compatible `gws` CLI.
