...
class PipelineTests(unittest.TestCase):
    ...
    def test_full_synthetic_demo_generates_all_artifacts(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(result["synthetic_acquisition"]["adapter"], "greenhouse_public_board")
            self.assertEqual(result["synthetic_acquisition"]["external_actions"], 1)
            self.assertEqual(result["synthetic_acquisition"]["vacancy_count"], 1)
            ...
            self.assertTrue((output / "gmail-triage.json").is_file())
            self.assertEqual(result["gmail_triage"]["status"], "proposed")
            self.assertEqual(result["gmail_reconciliation"]["status"], "dry_run")
            self.assertEqual(result["gmail_reconciliation"]["plan"]["decision"], "update_plan")
