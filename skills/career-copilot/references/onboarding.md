...
6. **Search strategy** — ask for optional target-company names and then show portal suggestions based on the declared role family, seniority, industries, eligible geography, work mode and employment type. Suggestions are transparent coverage heuristics, not a claim of current market performance or a directive; the candidate selects, adds or leaves portals blank. Each result includes a dated catalog version, evidence URL and narrow evidence scope; see `portal-catalog.md` (or `portal-catalog.es.md`). Keep targets and selected portal IDs only in the private rules file.
7. **Career direction** — optional success criteria, values, non-negotiables, tolerable tradeoffs, development gaps and a candidate-approved departure narrative. Keep facts, interpretations and preferences separate; blank stays unknown and does not filter roles.
8. **Compensation** — optional and disabled unless the user wants it considered.
9. **Permissions** — tracker update policy and external-action mode. `draft_only` is the default; `confirm_each_external` requires explicit opt-in.
10. **Integrations** — optional and disabled by default.
