...
5. After declared roles and eligible geography are available, use `status` to show transparent portal-coverage suggestions, then ask the optional target-company and selected-portal questions. Suggestions are not current-market rankings; each includes a dated catalog version, official evidence URL and narrow coverage scope. Read `references/portal-catalog.md` (or `references/portal-catalog.es.md`) before changing coverage rules. Keep chosen values only in the private workspace.
