...
2. Una exportación local JSON o CSV de vacantes elegida explícitamente puede generar un shortlist privado y de sólo lectura.
...
3. **Importación privada explícita:** `vacancy_discovery.py` lee únicamente la exportación local JSON o CSV indicada. El contrato completo y versionado de alias genéricos de encabezados CSV y etiquetas de fuente se publica en el [Inicio rápido](QUICKSTART.md#importa-una-exportación-privada-de-vacantes-sólo-lectura); no afirma compatibilidad con exportaciones de proveedores. Cada importación valida fuente seleccionada, frescura, URL canónica HTTP(S) y URLs canónicas duplicadas.
