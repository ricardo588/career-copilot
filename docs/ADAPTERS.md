...
```bash
# Add --apply to the reviewed command.
```
...
```bash
python3 "$ADAPTER" gmail-mark-read --message-id '<MESSAGE_ID>'
```

Apply by adding `--apply`. The adapter then confirms that the `UNREAD` label is absent.
